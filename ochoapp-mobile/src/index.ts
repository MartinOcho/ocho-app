...
app.post("/api/messages/:roomId/send-voice", async (req, res) => {
  try {
    const { user: loggedInUser, message: authMessage } = await getCurrentUser(req.headers);
    
    if (!loggedInUser) {
      return res.json({
        success: false,
        message: authMessage || "Utilisateur non authentifié.",
        name: "invalid_session",
      });
    }

    const { roomId } = req.params;
    const { voiceNoteUrl, duration } = req.body;

    if (!roomId || !voiceNoteUrl || !duration) {
      return res.json({
        success: false,
        message: "roomId, voiceNoteUrl et duration requis",
      });
    }

    // Vérifier que l'utilisateur est membre du room
    const membership = await prisma.roomMember.findUnique({
      where: { roomId_userId: { roomId, userId: loggedInUser.id } },
    });

    if (!membership) {
      return res.json({
        success: false,
        message: "Vous n'êtes pas membre de ce room",
      });
    }

    // Créer le message avec type VOICENOTE
    ...
    // Créer les MessageReads pour marquer comme lu pour l'expéditeur
    ...
    return res.json({
      success: false,
      error: "Impossible d'envoyer le message vocal",
      message: err instanceof Error ? err.message : undefined,
    } as ApiResponse<null>);
  }
});
